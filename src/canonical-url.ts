 ...
 *
 * `PUBLIC_URL` is written twice, once in `.env` for this service and once in
 ...
 * this package only trimmed it and stripped trailing slashes. `https://Mail.example.com`
 * on one side against `https://mail.example.com` on the other then answered 401
 * to every settings request, with nothing anywhere naming the cause.
 *
 * This file is the connector's copy. `oauth/src/canonical-url.ts` is the OAuth
 * layer's, and the two are identical below this comment; the drift test in
 * test/unit/canonical-url.test.ts fails if they part company, the same way
 * secrets.test.ts and settings-api.test.ts pin the two other mirrored pairs. The
 * packages have separate Docker build contexts and cannot import from one
 * another, so a mirrored module is the closest thing to a shared one they have.
 *
 * Only the canonicalisation is mirrored, not the rest of `oauth/src/urls.ts`:
 * redirect-URI matching and the Claude redirect allowlists are the authorization
 * server's own business and the connector has no use for them. `urls.ts` re-exports
 * the three functions below, so that package keeps one URL module and neither
 * package ends up holding two copies of the rule.
 */
